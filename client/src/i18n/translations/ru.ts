const ru: Record<string, string> = {
  ...
  'admin.smtp.port': 'SMTP-порт',
  'admin.smtp.user': 'Пользователь SMTP',
  'admin.smtp.pass': 'Пароль SMTP',
  'admin.smtp.from': 'Адрес отправителя',
  'admin.smtp.skipTls': 'Пропустить проверку TLS-сертификата',
  'admin.smtp.skipTlsHint': 'Включить для самоподписанных сертификатов на локальных почтовых серверах',
  ...
  'login.inviteExpired': 'Неверная или истёкшая ссылка приглашения',
  ...
  'admin.oidcDisplayNamePlaceholder': 'напр. Google, Authentik, Keycloak',
  'admin.oidcDiscoveryUrl': 'URL обнаружения',
  'admin.oidcDiscoveryUrlOptional': 'URL обнаружения (необязательно)',
  'admin.oidcDiscoveryUrlHint': 'Переопределяет автоматически созданный URL обнаружения. Необходимо для таких провайдеров, как Authentik.',
  ...
  'photos.countPlural': '{count} фотографий',
  'photos.countFor': '{count} фотографий для {trip}',
  'photos.linkDay': 'Привязать день',
  'photos.noDay': 'Без дня',
  ...
  'collab.chat.geminiExecute': 'Позволить Gemini выполнить',
  'collab.chat.geminiCommandHint': 'Выполнение Gemini начинается после отправки. Ответь на сообщение, чтобы использовать его как исходную идею.',
  'collab.chat.template.dayFlow': 'План дня',
  'collab.chat.template.dayFlowPrompt': 'Спланируй сегодняшний день как чёткое расписание с временными окнами, местами и 1 резервным вариантом.',
  'collab.chat.template.budgetPass': 'Бюджет',
  'collab.chat.template.budgetPassPrompt': 'Создай компактный бюджет с важнейшими статьями расходов, включая категорию и реалистичные цены.',
  'collab.chat.template.packingOptimizer': 'Оптимизатор упаковки',
  'collab.chat.template.packingOptimizerPrompt': 'Сгенерируй приоритетный список упаковки на основе погоды, активностей и продолжительности с количествами.',
  'collab.chat.template.mapExpansion': 'Расширение карты',
  'collab.chat.template.mapExpansionPrompt': 'Найди 5 подходящих мест для этой поездки, расставь приоритеты по логистической близости и добавляй только значимых кандидатов.',
  ...
  'notifications.count': '{count} уведомление',
  'notifications.countPlural': '{count} уведомлений',
